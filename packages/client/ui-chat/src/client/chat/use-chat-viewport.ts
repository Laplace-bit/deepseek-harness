...
export class ChatViewport {
  ...
  /**
   * Expose whether a third-party renderer claims the scrollport's programmatic bottom-follow
   * through the `data-follow-owned` marker. Automatic follow silences itself while claimed;
   * explicit navigation still writes and the owner observes the external handoff.
   * @returns whether the bound scrollport carries the ownership marker, or false while detached.
   */
  get externallyOwned(): boolean {
    return this.elements?.scroller.hasAttribute('data-follow-owned') ?? false
  }
