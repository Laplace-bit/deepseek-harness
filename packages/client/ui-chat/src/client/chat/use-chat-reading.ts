...
export class ChatReading {
  ...
  readonly onScroll = (scroll: ViewportScroll): void => {
    if ((!scroll.movedByReader && this.state.followingTail)
      ...
      if (!scroll.movedByReader && this.viewport.externallyOwned) {
        // The external owner delivers its own position; adopt it as the next attribution
        // baseline and keep the current follow state instead of writing the floor.
  ...
  onResize(): void {
    ...
    if (this.state.followingTail) {
      if (this.viewport.externallyOwned) return
      this.followTail()
  ...
  private readonly flushSample = (): void => {
    ...
    if (!scroll.movedByReader && followingTail && this.viewport.externallyOwned) {
      // The external owner owns tail writes; adopt the delivered position as the
      // next attribution baseline and keep the current follow state.
      this.viewport.acknowledge(scroll.metrics)
      return
    }
